  ...
  const instructors = [
    {
      ...
      students: 60,
      courses: 1,
      role: 'Project Manager & DevOps at',
      company: 'CODETREE',
      expertise: 'Web development, Software Engineering, Full-Stack Development, DevOps',
      joinDate: '1/15/2020',
      expertiseArray: ['Web Development', 'Software Engineering', 'Full-Stack Development', 'DevOps']
    },
    {
      id: 2,
      ...
      students: 10,
      courses: 3,
      role: 'Full-stack developer and UI/UX expert with a passion for creating beautiful, user-friendly web applications. Lead Software Engineer at',
      company: 'CODETREE',
      expertise: '',
      joinDate: '1/20/2025',
      expertiseArray: ['React', 'React.js', 'UI/UX Design', 'JavaScript', 'TypeScript', 'Node.js']
    }
  ]

  const filteredInstructors = instructors.filter((instructor) => {
    const matchesSearch = instructor.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         instructor.expertiseArray.some(exp => exp.toLowerCase().includes(searchQuery.toLowerCase()))
    const matchesExpertise = selectedExpertise === 'All Expertise' || 
                            instructor.expertiseArray.includes(selectedExpertise)
    return matchesSearch && matchesExpertise
  })

  const allExpertise = ['All Expertise', ...new Set(instructors.flatMap(i => i.expertiseArray))]
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-7xl mx-auto mt-12 px-4">
        ...
        <p className="mt-4 text-gray-600">Showing {filteredInstructors.length} of {instructors.length} instructors</p>
      </div>
        ...
        {filteredInstructors.map((instructor) => (
          <div key={instructor.id} className="bg-[#0D1117] rounded-xl overflow-hidden border border-[#1f2937] shadow-xl">
            ...
            <div className="bg-[#22c55e] p-4 flex items-center gap-3">
              <img src={instructor.image} className="w-12 h-12 rounded-full border-2 border-white shadow-md" alt={instructor.name} />
              <div>
                <h3 className="text-lg font-bold text-white">{instructor.name}</h3>
                <p className="text-white flex items-center gap-2 text-xs">
                  ⭐ {instructor.rating} <span className="text-gray-100">({instructor.students} students)</span>
                </p>
              </div>
            </div>
            ...
            <div className="p-4 text-gray-200">
              ...
              <p className="text-xs mb-4 tracking-wide leading-relaxed line-clamp-3">
                {instructor.role} <span className="font-semibold tracking-widest">{instructor.company}</span>
              </p>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-[#111827] p-4 rounded-lg text-center border border-gray-700">
                  ...
                  <p className="text-2xl font-bold mt-1">{instructor.students}</p>
                  <p className="text-xs">Students</p>
                </div>

                <div className="bg-[#111827] p-4 rounded-lg text-center border border-gray-700">
                  <i className="fa-solid fa-book text-green-400 text-lg"></i>
                  <p className="text-2xl font-bold mt-1">{instructor.courses}</p>
              ...
              {instructor.id === 1 ? (
                <div className="bg-[#0f172a] border border-gray-700 text-gray-300 px-3 py-2 mt-2 rounded-lg text-xs w-fit line-clamp-2">
                  {instructor.expertise}
                </div>
              ) : (
                <div className="flex flex-wrap gap-1.5">
                  {instructor.expertiseArray.slice(0, 3).map((exp, index) => (
                    <span key={index} className="bg-[#0f172a] border border-gray-700 px-3 py-1 rounded-lg text-xs">
                      {exp}
                    </span>
                  ))}
                  {instructor.expertiseArray.length > 3 && (
